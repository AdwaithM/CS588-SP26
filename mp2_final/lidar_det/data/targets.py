...
def encode_targets(
    ...
    grid_height, grid_width = bev_cfg.output_grid_size
    pixel_size = bev_cfg.output_resolution
    object_count = 0
    ...
    for i in range(len(boxes)):
        ...
        center_x, center_y, box_z = boxes[i, 0], boxes[i, 1], boxes[i, 2]
        box_length, box_width, box_height = boxes[i, 3], boxes[i, 4], boxes[i, 5]
        yaw = boxes[i, 6]
        
        u_float, v_float, is_inside = metric_to_output_grid(center_x, center_y, bev_cfg)

        if not is_inside:
            continue
        
        pixel_length = box_length / pixel_size
        pixel_width = box_width / pixel_size

        radius_pixels = gaussian_radius((pixel_width, pixel_length), target_cfg.gaussian_overlap)
        radius_pixels = max(target_cfg.min_gaussian_radius, int(radius_pixels))

        u_int, v_int = int(u_float), int(v_float)
        class_id = class_ids[i]

        draw_gaussian(heatmap[class_id], (u_int, v_int), radius_pixels)

        inds[object_count] = v_int * grid_width + u_int
        mask[object_count] = 1

        reg[object_count] = [u_float - u_int, v_float - v_int]

        height[object_count, 0] = box_z

        if target_cfg.use_log_dims:
            dims[object_count] = np.log(np.array([box_length, box_width, box_height]))
        else:
            dims[object_count] = np.array([box_length, box_width, box_height])


        rot[object_count] = np.array([np.sin(yaw), np.cos(yaw)])
        cls_targets[object_count] = class_id
...
def decode_targets(
    ...
    is_valid_object = encoded["mask"] == 1
    valid_indices = encoded["inds"][is_valid_object]
    ...
    row_indices = valid_indices // grid_width
    col_indices = valid_indices % grid_width

    offsets = encoded["reg"][is_valid_object]
    u_precise = col_indices.astype(np.float32) + offsets[:, 0]
    v_precise = row_indices.astype(np.float32) + offsets[:, 1]

    real_x, real_y = output_grid_to_metric(u_precise, v_precise, bev_cfg)
    real_z = encoded["height"][is_valid_object].flatten()

    raw_dims = encoded["dims"][is_valid_object]
    if target_cfg.use_log_dims:
        final_dims = np.exp(raw_dims)
    else:
        final_dims = raw_dims
    
    rotation_data = encoded["rot"][is_valid_object]
    final_yaw = np.arctan2(rotation_data[:, 0], rotation_data[:, 1])

    boxes = np.zeros((num_found, 7), dtype=np.float32)
    boxes[:, 0] = real_x
    boxes[:, 1] = real_y
    boxes[:, 2] = real_z
    boxes[:, 3:6] = final_dims
    boxes[:, 6] = final_yaw

    classes = encoded["cls_ids"][is_valid_object]
    scores = np.ones((num_found,), dtype=np.float32)
    # ======= STUDENT TODO END (do not change code outside this block) =======
...
def _nms_heatmap(heatmap: torch.Tensor, kernel: int = 3) -> torch.Tensor:
    ...
    keep_mask = (pooled == heatmap).float()

    suppressed_heatmap = heatmap * keep_mask
...
def decode_predictions(
    ...
    heatmap = torch.sigmoid(preds["heatmap"]).clamp(1e-4, 1 - 1e-4)

    heatmap = _nms_heatmap(heatmap)

    scores, flat_indices, class_ids, grid_y, grid_x = _topk(heatmap, k=topk)

    offsets = _transpose_and_gather_feat(preds["reg"], flat_indices)
    heights = _transpose_and_gather_feat(preds["height"], flat_indices)
    sizes = _transpose_and_gather_feat(preds["dims"], flat_indices)
    rotations = _transpose_and_gather_feat(preds["rot"], flat_indices)
    ...
    grid_height, _ = bev_cfg.output_grid_size
    resolution = bev_cfg.output_resolution

    x_coordinates = bev_cfg.x_min + grid_x  * resolution
    y_coordinates = bev_cfg.y_min + ((grid_height - 1) - grid_y) * resolution
    z_coordinates = heights[:, :, 0]
    ...
    for batch_index in range(batch_size):
        valid_mask = scores[batch_index] > score_threshold
        ...
        x_vals = x_coordinates[batch_index][valid_mask]
        y_vals = y_coordinates[batch_index][valid_mask]
        z_vals = z_coordinates[batch_index][valid_mask] 

        length_vals = box_length[batch_index][valid_mask]
        width_vals = box_width[batch_index][valid_mask]
        height_vals = box_height[batch_index][valid_mask]

        yaw_vals = yaw[batch_index][valid_mask]

        score_vals = scores[batch_index][valid_mask]
        class_vals = class_ids[batch_index][valid_mask]

        boxes = torch.stack(
            [x_vals, y_vals, z_vals, length_vals, width_vals, height_vals, yaw_vals],dim=1
        )

        out.append({
            "boxes": boxes.detach().cpu().numpy().astype(np.float32),
            "scores": score_vals.detach().cpu().numpy().astype(np.float32),
            "classes": class_vals.detach().cpu().numpy().astype(np.int64),
        })
