...
def rasterize_points_to_bev(points: np.ndarray, cfg: BEVConfig) -> np.ndarray:
    ...
    grid_height, grid_width = cfg.grid_size
    z_min = cfg.z_min
    z_max = cfg.z_max
    z_range = z_max - z_min

    pixel_rows, pixel_cols, is_within_xy_bounds = metric_to_grid(points[:, 0], points[:, 1], cfg)

    is_within_z_bounds = (points[:, 2] >= z_min) & (points[:, 2] <= z_max)

    valid_mask = is_within_xy_bounds & is_within_z_bounds
    valid_points = points[valid_mask]

    valid_pixel_rows = pixel_rows[valid_mask]
    valid_pixel_cols = pixel_cols[valid_mask]

    pixel_ids = valid_pixel_rows * grid_width + valid_pixel_cols
    total_pixels = grid_height * grid_width

    points_counts = np.bincount(pixel_ids, minlength=total_pixels)
    is_pixel_occupied = points_counts > 0

    max_heights = np.full(total_pixels, z_min, dtype=np.float32)
    np.maximum.at(max_heights, pixel_ids, valid_points[:, 2])

    sum_of_heights = np.bincount(pixel_ids, weights=valid_points[:, 2], minlength=total_pixels)
    sum_of_intensities = np.bincount(pixel_ids, weights=valid_points[:, 3], minlength=total_pixels)

    bev_image = np.zeros((4, grid_height, grid_width), dtype=np.float32)

    normalized_max_heights = (max_heights - z_min) / z_range
    bev_image[0] = normalized_max_heights.reshape(grid_height, grid_width)

    mean_heights = np.zeros(total_pixels, dtype=np.float32)
    mean_heights[is_pixel_occupied] = sum_of_heights[is_pixel_occupied] / points_counts[is_pixel_occupied]
    normalized_mean_heights = (mean_heights - z_min) / z_range
    bev_image[1] = normalized_mean_heights.reshape(grid_height, grid_width)

    mean_intensities = np.zeros(total_pixels, dtype=np.float32)
    mean_intensities[is_pixel_occupied] = sum_of_intensities[is_pixel_occupied] / points_counts[is_pixel_occupied]
    bev_image[2] = np.clip(mean_intensities, 0, 1).reshape(grid_height, grid_width)

    density_values = np.log1p(points_counts) / np.log1p(64)
    bev_image[3] = np.clip(density_values, 0, 1).reshape(grid_height, grid_width)

    bev_image[:, ~is_pixel_occupied.reshape(grid_height, grid_width)] = 0
